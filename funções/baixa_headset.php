...
try {
    ...
    $stmt = $pdo->prepare("UPDATE Funcionarios SET HeadDevolvido = 1, Lacre_devolvido = :lacre, DataBaixa = CAST(GETDATE() AS DATE) WHERE Matricula = :func_desl");
    ...
    $stmt2 = $pdo->prepare("UPDATE Headsets SET EmPosse = NULL, Estoque = 1, Manutencao = 0, Id_defeito = NULL, Inativo = 0, Treinamento = 0 WHERE EmPosse = :func_desl");
    ...
    Swal.fire({
        title: 'Erro!',
        text: 'Erro ao dar baixa! " . addslashes($e->getMessage()) . "',
        icon: 'error',
        timer: 2000,
        showConfirmButton: false
    }).then((result) => {
